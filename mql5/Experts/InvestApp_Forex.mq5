//+------------------------------------------------------------------+
//|                                           InvestApp_Forex.mq5    |
//|                         InvestApp – Forex Expert Advisor          |
//|                    Märkte: Major & Minor Forex-Paare              |
//|                    Phase 0 – Grundgerüst (März 2026)             |
//+------------------------------------------------------------------+
#property copyright "InvestApp"
#property version   "0.10"
#property description "InvestApp Forex EA – Phase 0 Grundgerüst"
#property strict
...
//--- Symbol-Liste Forex (Major + ausgewählte Minors)
string ForexSymbols[] = {
...
int OnInit()
{
   g_symbolCount = ArraySize(ForexSymbols);

   // TODO: Logger initialisieren (Logger.mqh)
   Print("[InvestApp_Forex] OnInit – EA startet. Symbole: ", g_symbolCount,
   ...
   if (!EventSetTimer(1)) {
      Print("[InvestApp_Forex] FEHLER: EventSetTimer fehlgeschlagen.");
      return INIT_FAILED;
   }

   g_initialized = true;
   Print("[InvestApp_Forex] Initialisierung abgeschlossen.");
...
void OnDeinit(const int reason)
{
   ...
   Print("[InvestApp_Forex] OnDeinit – EA gestoppt. Reason: ", reason);
}
...
void OnTimer()
{
   ...
   Print("[InvestApp_Forex] Analyse-Zyklus – ", TimeToString(now));

   // Alle Symbole iterieren
   for (int i = 0; i < g_symbolCount; i++)
   {
      string symbol = ForexSymbols[i];
      ...
      // if (!macroOk) { Print("[Forex] ", symbol, " – MacroFilter: BLOCK"); continue; }
      ...
      // if (!volOk) { Print("[Forex] ", symbol, " – VolatilityFilter: BLOCK"); continue; }
      ...
      // if (!validated) { Print("[Forex] ", symbol, " – Validation fehlgeschlagen"); continue; }
      ...
      // if (!risk.tradingAllowed) { Print("[Forex] ", symbol, " – RiskManager: Trade nicht zulässig"); continue; }
