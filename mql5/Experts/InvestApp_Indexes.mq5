...
//|                       InvestApp – Indexes Expert Advisor          |
//|                    Märkte: DAX, S&P 500, Dow Jones               |
//|                    Phase 0 – Grundgerüst (März 2026)             |
//+------------------------------------------------------------------+
#property copyright "InvestApp"
#property version   "0.10"
#property description "InvestApp Indexes EA – Phase 0 Grundgerüst"
#property strict
...
//--- Symbol-Liste Indexes
//    Symbolnamen können je nach Broker variieren – ggf. anpassen
string IndexesSymbols[] = {
   "GER40",   // DAX 40 (bei vielen Brokern: DE30, DE40, GER30)
   "US500",   // S&P 500 (SPX500, SP500)
   "US30"     // Dow Jones Industrial Average (DJ30, DJIA)
};
...
int OnInit()
{
   g_symbolCount = ArraySize(IndexesSymbols);

   // TODO: Logger initialisieren (Logger.mqh)
   Print("[InvestApp_Indexes] OnInit – EA startet. Symbole: ", g_symbolCount,
   ...
   if (!EventSetTimer(1)) {
      Print("[InvestApp_Indexes] FEHLER: EventSetTimer fehlgeschlagen.");
      return INIT_FAILED;
   }

   g_initialized = true;
   Print("[InvestApp_Indexes] Initialisierung abgeschlossen.");
...
void OnDeinit(const int reason)
{
   ...
   Print("[InvestApp_Indexes] OnDeinit – EA gestoppt. Reason: ", reason);
}
...
void OnTimer()
{
   ...
   Print("[InvestApp_Indexes] Analyse-Zyklus – ", TimeToString(now));

   // Alle Symbole iterieren
   for (int i = 0; i < g_symbolCount; i++)
   {
      string symbol = IndexesSymbols[i];

      // TODO: Prüfen ob Symbol verfügbar/aktiv ist
      ...
      // Indexes reagieren besonders stark auf US-Makrodaten → strenge Sperre
      // bool macroOk = MacroFilter_Check(symbol);
      // if (!macroOk) { Print("[Indexes] ", symbol, " – MacroFilter: BLOCK"); continue; }
      ...
      // Indexes haben höhere Volatilität – ATR-Schwellen entsprechend kalibrieren
      // bool volOk = VolatilityFilter_Check(symbol);
      // if (!volOk) { Print("[Indexes] ", symbol, " – VolatilityFilter: BLOCK"); continue; }
      ...
      // if (!validated) { Print("[Indexes] ", symbol, " – Validation fehlgeschlagen"); continue; }
      ...
      // if (!risk.tradingAllowed) { Print("[Indexes] ", symbol, " – RiskManager: Trade nicht zulässig"); continue; }
