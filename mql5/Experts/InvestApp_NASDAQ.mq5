...
void AnalyzeSymbol(string symbol)
{
   ...
   if(!IsTrendAligned(trend, /* signal_direction */ 0))
      LOG_D("InvestApp_NASDAQ", symbol, "Trend nicht ausgerichtet: " + trend.summary);

   if(trend.direction == TREND_NEUTRAL)
   {
      LOG_D("InvestApp_NASDAQ", symbol, "Kein klarer Trend: " + trend.summary);
      return;
   }
   ...
   if(!vol.isAcceptable)
   {
      LOG_D("InvestApp_NASDAQ", symbol, "Volatilität: " + vol.reject_reason);
      return;
   }
   if(!IsSessionActive(g_config))
   {
      LOG_D("InvestApp_NASDAQ", symbol, "Session nicht aktiv");
      return;
   }
   ...
   // if(!val.isValid) { LOG_W("InvestApp_NASDAQ", symbol, "Validierung: " + val.reject_reason); return; }
